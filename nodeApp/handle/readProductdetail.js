...
class ReadproductDetail extends BaseHandler {
     ...
     */
    async handle() {
        let configCollection = await this.mongodbService.getCollection( collectionName.productdetail);
        let readproduct=await configCollection.find().toArray();
        ...
        if(readproduct){
            this.webSocketUtil.sendToPage(MSG_TYPE.PAGE_READY_RESULT,readproduct)
        }
    }
